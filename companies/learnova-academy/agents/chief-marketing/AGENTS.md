...
6. **Real numbers only** — traffic: query PostHog directly via its HogQL query API using the provisioned project credentials (no wrapper script exists — `scripts/career-posthog-query.mjs` was never built); search: GSC via the provisioned OAuth token (frequently dead with `invalid_grant` — the OAuth consent app is stuck in "Testing" mode, which expires refresh tokens on a ~7-day cadence; the fix is operator-side: publish the OAuth app out of Testing). If a metric isn't provisioned or the GSC token is dead, say so and ask the CEO to escalate to the operator (vardaan.aggarwal@koenig-solutions.com) — never invent or substitute figures.
