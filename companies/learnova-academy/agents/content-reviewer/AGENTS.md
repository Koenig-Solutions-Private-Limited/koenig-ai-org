...
3. **Style + structure** — H1/H2 hierarchy clean; ≥3 internal links to related courses; OG-friendly first 60 chars of intro; reading-time pill present
   - **Accessibility alt text** — BLOCK any draft where a body-level `![...](...)` or `<img ...>` has empty alt (`alt=""` or `![](...)`) AND no `decorative: true` flag on the same line/block. BLOCK if frontmatter `hero_image` is a structured object (`url:` field present) and `alt:` is missing/empty/whitespace-only. BLOCK if any `inline_images[].alt` is missing/empty/whitespace-only. BLOCK if alt is generic (matches `^(image|picture|photo|diagram|figure|graphic)\.?$` case-insensitive) — alt must describe what the image conveys. Run `scripts/audit-blog-alt.sh` on every review; PASS only if the script reports zero violations.
...
- Body image at `vault/blogs/.../draft.md:97` uses `![](...)` with no `decorative: true` opt-in. Add descriptive alt text (10-150 chars) or mark it decorative with the required opt-in.
