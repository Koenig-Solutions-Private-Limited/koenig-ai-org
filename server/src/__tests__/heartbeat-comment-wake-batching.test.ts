...
describe("heartbeat comment wake batching", () => {
  ...
  it("skips passive deferred comment wakes when the active run closes the issue (KOEA-6165)", async () => {
    const gateway = await createControlledGatewayServer();
    const companyId = randomUUID();
    const agentId = randomUUID();
    const issueId = randomUUID();
    ...
    try {
      ...
      await db.insert(issues).values({
        id: issueId,
        companyId,
        title: "Stay done after passive deferred comment",
        status: "todo",
        priority: "medium",
        assigneeAgentId: agentId,
        issueNumber: 1,
      ...
      const skippedDeferredWake = await db
        .select({
        ...
        .then((rows) => rows.find((row) => row.reason === "terminal_issue_passive_comment_wake") ?? null);

      expect(skippedDeferredWake).toMatchObject({
        status: "skipped",
  ...
  it("promotes deferred explicit resume comment wakes and reopens a done issue", async () => {
    const gateway = await createControlledGatewayServer();
    const companyId = randomUUID();
    const agentId = randomUUID();
    const issueId = randomUUID();
    ...
    try {
      ...
      await db.insert(issues).values({
        id: issueId,
        companyId,
        title: "Explicit resume after deferred comment",
        status: "todo",
        priority: "medium",
        assigneeAgentId: agentId,
        issueNumber: 1,
      ...
      const comment2 = await db
        ...
        .values({
          ...
          issueId,
          authorUserId: "user-1",
          body: "Please resume this work after you finish",
        })
        .returning()
        .then((rows) => rows[0]);
      ...
      expect(String(secondPayload.message ?? "")).toContain("Please resume this work after you finish");
    } finally {
      gateway.releaseFirstWait();
      await gateway.close();
    }
  ...
  it("promotes deferred user comment wakes on blocked issues without changing status", async () => {
    const gateway = await createControlledGatewayServer();
    const companyId = randomUUID();
    const agentId = randomUUID();
    const issueId = randomUUID();
    ...
    try {
      ...
      await db.insert(issues).values({
        id: issueId,
        companyId,
        title: "Blocked issue deferred comment",
        status: "blocked",
        priority: "medium",
        assigneeAgentId: agentId,
        issueNumber: 1,
      ...
      const comment2 = await db
        ...
        .values({
          ...
          issueId,
          authorUserId: "user-1",
          body: "Follow-up while still blocked",
        })
        .returning()
        .then((rows) => rows[0]);
      ...
      expect(issueAfterPromotion?.status).toBe("blocked");
  ...
  it("does not reopen a finished issue when assignee closure supersedes explicit deferred resume (KOEA-5391)", async () => {
    const gateway = await createControlledGatewayServer();
    const companyId = randomUUID();
    const agentId = randomUUID();
    const issueId = randomUUID();
    ...
    try {
      ...
      await db.insert(issues).values({
        id: issueId,
        companyId,
        title: "Stay done after assignee reconciliation",
        status: "todo",
        priority: "medium",
        assigneeAgentId: agentId,
        issueNumber: 1,
      ...
      const deferredUserComment = await db
        .insert(issueComments)
        .values({
          companyId,
          issueId,
      ...
      const deferredRun = await heartbeat.wakeup(agentId, {
        ...
        payload: {
          issueId,
          commentId: deferredUserComment.id,
          resumeIntent: true,
          followUpRequested: true,
        },
        contextSnapshot: {
      ...
      await db
        ...
        .values({
          ...
          authorAgentId: agentId,
          createdByRunId: firstRun?.id ?? null,
          body: "Done — reconciled and closing this issue.",
        });
      ...
      const deferredWakeReopenActivities = await db
        .select({
      ...
      expect(
        deferredWakeReopenActivities.some(
          (entry) =>
            typeof entry.details === "object" &&
            entry.details !== null &&
            (entry.details as Record<string, unknown>).source === "deferred_comment_wake",
        ),
      ).toBe(false);
      ...
      expect(secondPayload.paperclip).toMatchObject({
        wake: {
          reason: "issue_reopened_via_comment",
          commentIds: [deferredUserComment.id],
          latestCommentId: deferredUserComment.id,
          issue: {
            id: issueId,
            identifier: `${issuePrefix}-1`,
            title: "Stay done after assignee reconciliation",
