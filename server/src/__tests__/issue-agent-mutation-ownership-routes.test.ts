...
describe("agent issue mutation checkout ownership", () => {
  ...
  function mockIssuesCommentAnyGrant() {
    mockAccessService.hasPermission.mockImplementation(async (
      _companyId: string,
      _principalType: string,
      principalId: string,
      permissionKey: string,
    ) => principalId === peerAgentId && permissionKey === "issues:comment_any");
  }

  it("allows a granted peer agent to add inert comments across assignees", async () => {
    mockIssueService.getById.mockResolvedValue(makeIssue({ status: "todo", assigneeAgentId: ownerAgentId }));
    mockIssuesCommentAnyGrant();

    const res = await request(await createApp(peerActor()))
      .post(`/api/issues/${issueId}/comments`)
      .send({ body: "watchdog operational nudge" });

    expect(res.status, JSON.stringify(res.body)).toBe(201);
    expect(mockIssueService.addComment).toHaveBeenCalledWith(
      issueId,
      "watchdog operational nudge",
  ...
  ])("does not allow %s with issues:comment_any grant only", async (_name, sendRequest) => {
    mockIssueService.getById.mockResolvedValue(makeIssue({ status: "todo", assigneeAgentId: ownerAgentId }));
    mockIssuesCommentAnyGrant();
  ...
  ])("does not allow comment %s side effects with issues:comment_any grant only", async (_name, payload) => {
    mockIssueService.getById.mockResolvedValue(makeIssue({ status: "done", assigneeAgentId: ownerAgentId }));
    mockIssuesCommentAnyGrant();

    const res = await request(await createApp(peerActor()))
      .post(`/api/issues/${issueId}/comments`)
      .send({ body: "attempted side effect", ...payload });

    expect(res.status, JSON.stringify(res.body)).toBeGreaterThanOrEqual(403);
    expect(res.status).toBeLessThan(500);
    expect(mockIssueService.addComment).not.toHaveBeenCalled();
    expect(mockIssueService.update).not.toHaveBeenCalled();
  });

  it("denies cross-company comment even when issues:comment_any would otherwise match", async () => {
    mockIssuesCommentAnyGrant();
