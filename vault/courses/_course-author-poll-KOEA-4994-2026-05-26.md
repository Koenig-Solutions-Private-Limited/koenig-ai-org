...
type: course-draft
ticket: KOEA-4994
paperclip_issue_id: 02a9d4b1-8b31-4ae1-ac24-25e7f63c1317
...
# Course Author Poll - KOEA-4994 - 2026-05-26

## Wake context

KOEA-4994 was assigned to Course Author with checkout already claimed by the harness. The inline wake payload had:
...
The requested `course-architect` workflow is available in this repo at `companies/learnova-academy/skills/course-architect/SKILL.md`, but it requires a new-course ticket with a course slug, audience, vendor tag, level, and priority. KOEA-4994's inline payload does not include those inputs.

## Runtime blocker

The Paperclip control-plane API was not reachable from this heartbeat:

- `GET $PAPERCLIP_API_URL/api/issues/KOEA-4994/heartbeat-context` failed with connection refused at `http://localhost:3100`.
- `GET http://localhost:3100/api/health`, `http://localhost:3101/api/health`, `http://localhost:3102/api/health`, and `http://localhost:3000/api/health` all failed with connection refused.

Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, create child issues, or patch KOEA-4994 to `blocked` through the normal Paperclip route.
...
| `mcp-from-first-principles-to-production` | `g3-passed` | Eligible only if a ticket names the exact chapter, revision, or delta. Existing chapter files are already present. |
...
No outline or chapter file was created or edited. KOEA-4994's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-4994's missing course brief inline.

When the API is reachable, Course Author should update KOEA-4994 with this poll result and either:

1. close KOEA-4994 as a completed poll if it has no specific Course Author deliverable, or
