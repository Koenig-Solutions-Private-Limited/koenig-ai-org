...
type: course-draft
ticket: KOEA-5932
paperclip_issue_id: 3d1702c4-ea21-4e1f-bdd9-02347ccf7bef
...
# Course Author Poll - KOEA-5932 - 2026-05-27

## Wake context

KOEA-5932 was assigned to Course Author with checkout already claimed by the harness. The inline wake payload had:
...
The requested `course-architect` workflow exists in this repo at `companies/learnova-academy/skills/course-architect/SKILL.md`, but it requires a new-course ticket with a course slug, target audience, vendor tag, level, priority, and strategic brief. KOEA-5932's inline payload does not include those inputs.

The requested `course-chapter-write` workflow exists in this repo at `companies/learnova-academy/skills/course-chapter-write/SKILL.md`, but it requires a concrete chapter assignment plus a matching outline that is already `g0-passed`. KOEA-5932's inline payload does not include a course slug, chapter number, chapter focus, or approved outline reference.
...
Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, create child issues, or patch KOEA-5932 to `blocked` through the normal Paperclip route.
...
The bounded scan found 132 visible markdown or YAML files under `vault/courses/`, including course outlines, chapter drafts, metadata files, and prior poll notes. KOEA-5932 does not identify any of them as this heartbeat's assignment, so they are review/approval pipeline artifacts, not permission to create or revise chapter content.

## Decision

No outline or chapter file was created or edited. KOEA-5932's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-5932's missing course brief inline.

When the API is reachable, Course Author should update KOEA-5932 with this poll result and either:

1. close KOEA-5932 as a completed poll if it has no specific Course Author deliverable, or
