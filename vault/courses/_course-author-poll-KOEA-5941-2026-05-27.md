...
type: course-draft
ticket: KOEA-5941
paperclip_issue_id: f97c240e-8823-4af4-83e7-f0a5c3643f41
...
# Course Author Poll - KOEA-5941 - 2026-05-27

## Wake context

KOEA-5941 was assigned to Course Author with checkout already claimed by the harness. The inline wake payload had:
...
The requested `course-architect` workflow exists in this repo at `companies/learnova-academy/skills/course-architect/SKILL.md`, but it requires a new-course ticket with a course slug, target audience, vendor tag, level, priority, and strategic brief. KOEA-5941's inline payload does not include those inputs.

The requested `course-chapter-write` workflow exists in this repo at `companies/learnova-academy/skills/course-chapter-write/SKILL.md`, but it requires a concrete chapter assignment plus a matching outline that is already `g0-passed`. KOEA-5941's inline payload does not include a course slug, chapter number, chapter focus, or approved outline reference.
...
Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, create child issues, or patch KOEA-5941 to `blocked` through the normal Paperclip route.
...
The bounded scan found 135 visible markdown or YAML files under `vault/courses/`, including course outlines, chapter drafts, metadata files, and prior poll notes. KOEA-5941 does not identify any of them as this heartbeat's assignment, so they are review/approval pipeline artifacts, not permission to create or revise chapter content.

## Decision

No outline or chapter file was created or edited. KOEA-5941's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-5941's missing course brief inline.

When the API is reachable, Course Author should update KOEA-5941 with this poll result and either:

1. close KOEA-5941 as a completed poll if it has no specific Course Author deliverable, or
