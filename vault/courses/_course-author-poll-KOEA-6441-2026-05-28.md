...
type: course-draft
ticket: KOEA-6441
paperclip_issue_id: cf6cc470-98d8-4b1f-9fe6-28ac4f8f637f
...
# Course Author Poll - KOEA-6441 - 2026-05-28

## Wake context

KOEA-6441 was assigned to Course Author with checkout already claimed by the harness. The inline wake payload had:
...
The requested `course-architect` workflow applies only when a new-course ticket supplies a course slug, target audience, prerequisites, outcomes, level, and strategic brief. KOEA-6441's inline payload does not include those inputs.

The requested `course-chapter-write` workflow applies only when an approved outline is already `g0-passed` and the ticket names the exact chapter assignment. KOEA-6441's inline payload does not include a course slug, chapter number, chapter focus, or approved outline reference.
...
Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, create child issues, or patch KOEA-6441 to `blocked` through the normal Paperclip route.
...
| `claude-mcp-mastery` | `g0-passed` | Eligible only if a ticket names the exact chapter, revision, or delta. |
...
The bounded scan found no files at the expected Stage 2 chapter path shape `vault/courses/<slug>/<chN>-<topic>/chapter.md`. KOEA-6441 does not identify any visible course artifact as this heartbeat's assignment, so existing outlines are review/approval pipeline artifacts, not permission to create or revise chapter content.

## Decision

No outline or chapter file was created or edited. KOEA-6441's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-6441's missing course brief inline.

When the API is reachable, Course Author should update KOEA-6441 with this poll result and either:

1. close KOEA-6441 as a completed poll if it has no specific Course Author deliverable, or
