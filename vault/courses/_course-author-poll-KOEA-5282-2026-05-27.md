...
type: course-draft
ticket: KOEA-5282
paperclip_issue_id: 7c840504-0345-491a-9911-5f19a25e7ff3
...
# Course Author Poll - KOEA-5282 - 2026-05-27

## Wake context

KOEA-5282 was assigned to Course Author with checkout already claimed by the harness. The inline wake payload had:
...
The requested `course-architect` workflow is available in this repo at `companies/learnova-academy/skills/course-architect/SKILL.md`, but it requires a new-course ticket with a course slug, target audience, vendor tag, level, and priority. KOEA-5282's inline payload does not include those inputs.

The requested `course-chapter-write` workflow is available in this repo at `companies/learnova-academy/skills/course-chapter-write/SKILL.md`, but it requires a concrete chapter assignment plus a matching outline that is already `g0-passed`. KOEA-5282's inline payload does not include a course slug, chapter number, or chapter focus.

## Runtime blocker

The Paperclip control-plane API was not reachable from this heartbeat:

- `GET $PAPERCLIP_API_URL/api/issues/KOEA-5282/heartbeat-context` failed with connection refused at `http://localhost:3100`.
- `GET http://127.0.0.1:3100/api/health` failed with connection refused.

Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, create child issues, or patch KOEA-5282 to `blocked` through the normal Paperclip route.
...
There are visible chapter drafts and finished chapters in the vault, but KOEA-5282 does not identify any chapter requiring author action. A poll wake should not modify them without a named course-delta or reviewer request.

## Decision

No outline or chapter file was created or edited. KOEA-5282's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-5282's missing course brief inline.

When the API is reachable, Course Author should update KOEA-5282 with this poll result and either:

1. close KOEA-5282 as a completed poll if it has no specific Course Author deliverable, or
