...
type: course-draft
ticket: KOEA-5536
paperclip_issue_id: 2b067665-ca29-4ad4-963d-b23d9410313b
...
# Course Author Poll - KOEA-5536 - 2026-05-27

## Wake context

KOEA-5536 was assigned to Course Author with checkout already claimed by the harness. The inline wake payload had:
...
Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, create child issues, or patch KOEA-5536 to `blocked` through the normal Paperclip route.
...
Visible chapter drafts and course artifacts may exist below these folders, but KOEA-5536 does not identify any specific chapter requiring Course Author action.

## Decision

No outline or chapter file was created or edited. KOEA-5536's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-5536's missing course brief inline.

When the API is reachable, Course Author should update KOEA-5536 with this poll result and either:

1. close KOEA-5536 as a completed poll if it has no specific Course Author deliverable, or
