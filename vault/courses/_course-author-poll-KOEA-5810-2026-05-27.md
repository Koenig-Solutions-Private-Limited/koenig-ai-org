...
type: course-draft
ticket: KOEA-5810
paperclip_issue_id: 3b2dc55a-3ca6-4aa0-941b-e5e4101421d0
...
# Course Author Poll - KOEA-5810 - 2026-05-27

## Wake context

KOEA-5810 was assigned to Course Author with checkout already claimed by the harness. The inline wake payload had:
...
The requested `course-architect` workflow requires a new-course ticket with a course slug, target audience, level, prerequisites, learning outcomes, and strategic brief. KOEA-5810's inline payload does not include those inputs.

The requested `course-chapter-write` workflow requires a concrete chapter assignment plus a matching outline that is already `g0-passed`. KOEA-5810's inline payload does not include a course slug, chapter number, chapter focus, or approved outline reference.
...
Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, create child issues, or patch KOEA-5810 to `blocked` through the normal Paperclip route.
...
No `vault/courses/**/chapter.md` files are visible in the bounded scan. Several courses have nonstandard chapter markdown already, but KOEA-5810 does not identify any approved course chapter, revision, or delta requiring Course Author action.

## Decision

No outline or chapter file was created or edited. KOEA-5810's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-5810's missing course brief inline.

When the API is reachable, Course Author should update KOEA-5810 with this poll result and either:

1. close KOEA-5810 as a completed poll if it has no specific Course Author deliverable, or
