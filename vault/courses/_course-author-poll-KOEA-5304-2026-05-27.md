...
type: course-draft
ticket: KOEA-5304
paperclip_issue_id: db3f1b59-edfa-42c8-bc51-6bee049d1977
...
# Course Author Poll - KOEA-5304 - 2026-05-27

## Wake context

KOEA-5304 was assigned to Course Author with checkout already claimed by the harness. The inline wake payload had:
...
Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, create child issues, or patch KOEA-5304 to `blocked` through the normal Paperclip route.
...
Visible chapter drafts and finished chapters exist for some courses, but KOEA-5304 does not identify any chapter requiring Course Author action. A poll wake should not modify them without a named course-delta or reviewer request.

## Decision

No outline or chapter file was created or edited. KOEA-5304's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-5304's missing course brief inline.

When the API is reachable, Course Author should update KOEA-5304 with this poll result and either:

1. close KOEA-5304 as a completed poll if it has no specific Course Author deliverable, or
