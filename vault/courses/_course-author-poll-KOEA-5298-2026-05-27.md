...
type: course-draft
ticket: KOEA-5298
paperclip_issue_id: 5fa934c6-44ca-49d6-a01e-70c2b6cc4fe1
...
# Course Author Poll - KOEA-5298 - 2026-05-27

## Wake context

KOEA-5298 was assigned to Course Author with checkout already claimed by the harness. The inline wake payload had:
...
Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, create child issues, or patch KOEA-5298 to `blocked` through the normal Paperclip route.
...
Visible chapter files also exist for some courses whose outlines currently show `awaiting-g0`; those are review/approval pipeline artifacts, not permission to create or revise chapters without a named chapter/delta ticket.

## Decision

No outline or chapter file was created or edited. KOEA-5298's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-5298's missing course brief inline.

When the API is reachable, Course Author should update KOEA-5298 with this poll result and either:

1. close KOEA-5298 as a completed poll if it has no specific Course Author deliverable, or
