...
type: course-draft
ticket: KOEA-5066
paperclip_issue_id: a3aa4572-12a4-472e-b061-bd921fb42ec8
...
# Course Author Poll - KOEA-5066 - 2026-05-26

## Wake context

KOEA-5066 was assigned to Course Author with checkout already claimed by the harness. The inline wake payload had:
...
The requested `course-architect` workflow is available in this repo at `companies/learnova-academy/skills/course-architect/SKILL.md`, but it requires a new-course ticket with a course slug, target audience, vendor tag, level, and priority. KOEA-5066's inline payload does not include those inputs.
...
Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, create child issues, or patch KOEA-5066 to `blocked` through the normal Paperclip route.
...
No outline or chapter file was created or edited. KOEA-5066's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-5066's missing course brief inline.

When the API is reachable, Course Author should update KOEA-5066 with this poll result and either:

1. close KOEA-5066 as a completed poll if it has no specific Course Author deliverable, or
