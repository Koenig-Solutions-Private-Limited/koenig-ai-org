...
# Course Author Poll - 2026-05-26

## Wake context

KOEA-4893 was assigned to Course Author with the issue already checked out by the harness. The wake payload had:
...
Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, or patch KOEA-4893 to `blocked` through the normal Paperclip route.
...
No outline or chapter file was created or edited. KOEA-4893's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-4893's missing course brief inline.

When the API is reachable, Course Author should update KOEA-4893 with this poll result and either:

1. close KOEA-4893 as a completed poll if it has no specific Course Author deliverable, or
