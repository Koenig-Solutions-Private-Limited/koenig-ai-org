...
quiz:
  - question: "What does setting tool_choice: 'auto' in session.update instruct the Realtime API model to do?"
    options:
      - "Call a tool on every single response turn without exception or context evaluation"
      - "Let the user select which tool to invoke interactively during the live voice session"
      - "Decide contextually whether a tool call is appropriate for each given turn"
      - "Disable tool calling and produce audio output responses only for every turn"
    correct_idx: 2
    explanation: "'auto' means the model applies its reasoning to decide whether a tool call is appropriate for each turn. Setting tool_choice to a specific function name forces that tool on every response. 'none' disables tools entirely."
    section_anchor: "registering-tools-in-a-realtime-session"
  - question: "Why must you fire dispatchTool() without await inside the WebSocket message handler in Node.js?"
    options:
      - "The WebSocket library does not allow async functions as message event handlers at all"
      - "Awaiting blocks Node.js's single-threaded event loop, preventing incoming audio events from processing"
      - "The Realtime API closes the connection if no new message is sent within 100 milliseconds"
      - "Awaited tool results are silently discarded by the Realtime API before injection can occur"
    correct_idx: 1
    explanation: "Node.js runs on a single-threaded event loop. Awaiting inside the handler blocks it for the tool's duration, preventing processing of input_audio_buffer.speech_started barge-in signals and new audio chunks. Fire the async function without awaiting so the loop stays free."
    section_anchor: "non-blocking-tool-dispatch-the-critical-constraint"
  - question: "You send conversation.item.create with a function_call_output item but omit the subsequent response.create event. What happens?"
    options:
      - "The model detects the new tool result automatically and begins generating audio output"
      - "The result is stored in conversation history but the model produces no verbal response turn"
      - "The Realtime API returns a 400 validation error requiring an explicit response.create call"
      - "The WebSocket connection closes because the unacknowledged tool result times out server-side"
    correct_idx: 1
    explanation: "function_call_output adds the result to context but does not start a model turn. Without response.create, the model waits indefinitely. The turn never begins and the user hears silence. Always send both events back-to-back."
    section_anchor: "injecting-tool-results-back-into-the-session"
  - question: "A tool call throws an unexpected exception during a live voice session. What is the safest response pattern?"
    options:
      - "Re-raise the error and let Node's unhandledRejection handler log it to the console"
      - "Catch the error and inject a function_call_output with a plain-text error description"
      - "Close the WebSocket session cleanly and prompt the user to reconnect and try again"
      - "Retry the tool call up to three times before eventually returning silence to the user"
    correct_idx: 1
    explanation: "Always inject a result — even an error description — so the model has something to verbalize. An uninjected call_id leaves the model waiting for a result that never arrives. The model naturally produces graceful verbal recovery from error outputs like { error: 'Service unavailable.' }."
    section_anchor: "handling-tool-errors-gracefully"
---
