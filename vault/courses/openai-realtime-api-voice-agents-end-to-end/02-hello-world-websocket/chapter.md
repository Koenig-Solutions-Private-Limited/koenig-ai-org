...
chapter_number: 2
chapter_slug: hello-world-websocket
...
duration_min: 55
prerequisites:
  - "Chapter 1: The Voice Agent Landscape"
tags: [openai, realtime-api, voice-agents, WebSocket, VAD, turn-detection, audio-streaming]
chapter_primary_query: "how to build a WebSocket voice agent with OpenAI Realtime API"
first_60_words_answer: "Connect a persistent WebSocket to the OpenAI Realtime API, stream microphone audio in real time, and play back speech deltas as they arrive — with under 700 ms response latency from end of speech to first audio byte. This chapter delivers that session end-to-end: continuous 24 kHz PCM streaming, event-driven response handling, server-side VAD for automatic turn detection, barge-in cancellation via response.cancel, and a PTT fallback you can toggle at runtime without reconnecting."
positions: []  # No stances in STANCES.md v2026-06-02 apply to voice agent WebSocket transport
faq:
  - question: "Should I use server VAD or push-to-talk (PTT) for a production voice agent?"
    answer: |
      Ship both. Server VAD eliminates UI friction and delivers under 700 ms latency
      in quiet environments, but false positives in noisy settings — or when users
      pause mid-sentence — will interrupt the speaker prematurely. PTT adds roughly
      150 ms latency per turn (the key-release delay) but never misfires. Recommended
      pattern: default to VAD with `silence_duration_ms: 700`, expose a clearly
      labelled PTT toggle, and let real user testing determine the right default for
      your deployment context.
      [(OpenAI, "Voice Activity Detection (VAD)", developers.openai.com, retrieved 2026-06-15)](https://developers.openai.com/api/docs/guides/realtime-vad)
  - question: "How do I tune turn_detection parameters for a noisy call-center environment?"
    answer: |
      Raise `threshold` from the default 0.5 to 0.65–0.7 so the VAD requires higher
      audio energy before classifying a frame as speech — this filters keyboard
      clicks and background chatter. Raise `silence_duration_ms` from 500 to
      700–800 ms to prevent the agent responding during mid-sentence pauses. Both
      parameters can be updated mid-session via `session.update` without
      reconnecting. Always test with actual recordings from the deployment
      environment; synthetic silence does not reflect real-world SNR conditions.
      [(OpenAI Realtime API, "Client Events", developers.openai.com, retrieved 2026-06-15)](https://developers.openai.com/api/reference/resources/realtime/client-events)
  - question: "Why must I send response.cancel on barge-in, and what happens if I don't?"
    answer: |
      When `input_audio_buffer.speech_started` fires while the agent is mid-response,
      `response.cancel` aborts the in-flight generation immediately. Without it, the
      model keeps producing audio tokens no one is listening to, draining the
      128 k-token context window and leaving the audio output buffer out of sync with
      what the user hears. `response.cancel` is idempotent — safe to send even when
      no response is active. Cancelled response items are not written to conversation
      history, so context stays clean for the next turn.
      [(OpenAI Realtime API, "Client Events", developers.openai.com, retrieved 2026-06-15)](https://developers.openai.com/api/reference/resources/realtime/client-events)
---
...
The conversation history is stored server-side across the session's 128,000-token context window [(OpenAI, "Realtime Sessions", developers.openai.com, retrieved 2026-06-15)](https://developers.openai.com/api/reference/resources/realtime/sessions). Audio consumes approximately 800 tokens per minute [(OpenAI, "Realtime Guide", developers.openai.com, retrieved 2026-06-15)](https://developers.openai.com/api/docs/guides/realtime) — at the June 2026 list price of $32/1M audio-input tokens [(OpenAI, "API Pricing", openai.com, retrieved 2026-06-15)](https://openai.com/api/pricing) that is roughly $0.026 per minute of audio input — so a typical 5-minute conversation uses around 4,000 audio tokens of context — leaving plenty of headroom for tool results and system instructions. For the event-driven dispatch patterns that extend naturally to multi-agent orchestration, see [[claude-mcp-mastery]] — the async event handling mental model transfers directly.

<KnowledgeCheck>
**Q:** You send `input_audio_buffer.append` events continuously but the agent responds immediately after your first word, before you finish speaking. What is the most likely cause?

a) The `input_audio_buffer.commit` event was sent automatically by a separate code path  
b) The `max_response_output_tokens` value is set too low  
c) Server VAD detected speech ending early because `threshold` is set too low ✓  
d) The audio chunk size is too large, causing buffer overflow

**Why:** When `turn_detection.threshold` is too low (e.g., 0.3), brief dips in volume during normal speech — pausing between words — register as silence and trigger a premature commit. Raise `threshold` to 0.6–0.7 to require more sustained silence before the VAD fires.
</KnowledgeCheck>
...
<Callout type="info">
**Semantic VAD (preview):** OpenAI's turn-detection docs also include a `semantic_vad` mode alongside `server_vad`. It uses model-layer understanding of utterance completion rather than raw audio energy thresholds, reducing false positives in conversational pauses. `semantic_vad` is not covered in this chapter; enable it with `"type": "semantic_vad"` in `turn_detection`. [(OpenAI, "Voice Activity Detection", developers.openai.com, retrieved 2026-06-15)](https://developers.openai.com/api/docs/guides/realtime-vad)
</Callout>
...
<KnowledgeCheck>
**Q:** You raise `silence_duration_ms` from 500 to 1200 ms to reduce VAD false positives for a noisy environment. What observable trade-off does the user experience?

a) The agent's audio output quality drops because of buffer overflow  
b) The agent's response latency increases by approximately 700 ms on every turn ✓  
c) Barge-in interruptions no longer cancel the in-flight response  
d) The `conversation.item.created` event stops firing after each user turn

**Why:** `silence_duration_ms` is dead time before the model can begin processing — every turn must wait for the full silence window to close before the buffer commits. Adding 700 ms to this window adds 700 ms of floor latency to every response.
</KnowledgeCheck>
...
<KnowledgeCheck>
**Q:** An agent response consistently ends mid-sentence with the audio cutting off abruptly. Inspecting events, you see `response.done` with `status: "incomplete"`. What is the direct cause?

a) The WebSocket connection dropped and the server closed the stream  
b) The `silence_duration_ms` VAD window fired during the response  
c) The response exceeded `max_response_output_tokens` ✓  
d) The `input_audio_format` does not match the server's expected format

**Why:** `status: "incomplete"` signals that the model hit the `max_response_output_tokens` cap before finishing its output. Increase the cap or add explicit response-length constraints in your system prompt.
</KnowledgeCheck>
