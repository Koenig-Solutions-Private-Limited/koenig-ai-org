...
owns:
  ...
  - "session lifecycle management: timeouts, reconnects, orphan session cleanup"
  - "PII redaction from voice transcripts at write time"
  - "audit logging schema and retention policy for tool calls"
  - "per-session token budgets and model fallback on quota exhaustion"
defers_to:
  - "non-blocking tool dispatch patterns → ch03"
  - "latency profiling and optimization techniques → ch04"
  - "cost modeling across architectures → ch06"
quiz_topics:
  - "why round-robin load balancing breaks WebSocket voice sessions"
  - "what a server must do when a client reconnects after a network drop"
  - "when PII redaction must be applied to transcripts"
  - "which Realtime API event carries per-turn token usage data"
quiz:
  - question: "Why does round-robin load balancing break WebSocket voice sessions?"
    options:
      - "WebSocket connections require HTTPS, which standard round-robin load balancers do not directly support"
      - "Round-robin can route a reconnect to a different server, which holds none of the live session state"
      - "The OpenAI Realtime API aggressively rate-limits connections originating from too many upstream server IPs"
      - "Round-robin by default terminates idle WebSocket connections silently after 60 seconds as a protocol enforcement"
    correct_idx: 1
    ...
    section_anchor: "why-stateless-scaling-breaks-voice-sessions"
  - question: "What must a server do when a client reconnects after a network drop with the OpenAI Realtime API?"
    options:
      ...
      - "Re-inject all stored conversation history via conversation.item.create before the user speaks their first utterance"
    ...
    section_anchor: "session-lifecycle-management"
  - question: "When should PII redaction be applied to voice session transcripts?"
    ...
    section_anchor: "pii-redaction-from-transcripts"
  - question: "Which Realtime API server event carries the per-turn token usage data you need for budget tracking?"
    options:
      ...
      - "response.audio_transcript.done — includes a complete text transcript but carries no token count usage data"
    correct_idx: 1
    ...
    section_anchor: "per-session-token-budgets-and-model-fallback"
---
