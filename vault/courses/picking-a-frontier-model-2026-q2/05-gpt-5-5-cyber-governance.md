...
positions: []
quiz:
  - question: "What does TAC (Trusted Access for Cyber) verification actually change about GPT-5.5 access for approved security teams?"
    options:
      - "TAC upgrades the underlying GPT-5.5-Cyber model to a more capable version unavailable on the standard API"
      - "TAC changes the output policy layer, routing specific cyber-adjacent categories (vulnerability triage, patch authoring) to a monitored approval path rather than blocking them"
      - "TAC automatically grants Bedrock endpoint access with CloudTrail logging and VPC routing pre-configured"
      - "TAC verification cross-recognizes Glasswing eligibility, giving access to both GPT-5.5-Cyber and Claude Mythos Preview"
    correct_idx: 1
    explanation: "GPT-5.5-Cyber is the same underlying model; what TAC changes is the output policy applied to requests from verified teams. Outputs that would be blocked or truncated for a standard API customer in specific cyber-adjacent categories are instead routed to a monitored approval path or allowed with logging for TAC-verified teams. Exploit weaponization remains blocked regardless of TAC status. TAC does not modify Bedrock configuration, and it is not cross-recognized by Anthropic."
    section_anchor: the-trusted-access-for-cyber-program
  - question: "A TAC-verified team calls GPT-5.5-Cyber through Amazon Bedrock for FedRAMP-adjacent data residency compliance. Which statement correctly describes the governance picture?"
    options:
      - "Bedrock delivery automatically applies the TAC-relaxed output policy because AWS manages verification at the IAM control plane layer"
      - "TAC eligibility is enforced at the OpenAI application layer regardless of delivery path; Bedrock adds endpoint controls (IAM, VPC, CloudTrail) but not model access eligibility"
      - "Bedrock delivery voids TAC eligibility because the API call is proxied through AWS infrastructure rather than OpenAI-direct servers"
      - "Bedrock substitutes for TAC for FedRAMP environments; AWS IAM certification satisfies OpenAI's verification requirements"
    correct_idx: 1
    explanation: "Bedrock adds endpoint governance — IAM role-based access, VPC private routing, CloudTrail logging, regional data residency — layered on top of OpenAI's platform controls. But TAC eligibility is determined at the OpenAI application layer and applies regardless of which delivery path reaches it. An unverified team calling through Bedrock still receives the standard output policy. Bedrock is compliance-additive, not a substitute for model access verification."
    section_anchor: deployment-paths-openai-direct-enterprise-and-amazon-bedrock
  - question: "As of Q2 2026, how does a security team gain access to Claude Mythos Preview through Anthropic's Project Glasswing?"
    options:
      - "By submitting an application through the publicly available Glasswing Cyber Verification Program portal at anthropic.com/glasswing"
      - "By holding active TAC verification from OpenAI, which Anthropic cross-recognizes for Glasswing eligibility"
      - "Access is currently limited to approximately 50 invited organizational partners; no open application pathway for the CVP exists"
      - "By purchasing an Anthropic Enterprise contract, which includes Mythos Preview access as a standard tier add-on"
    correct_idx: 2
    explanation: "As of Q2 2026, Anthropic's Cyber Verification Program (CVP) is announced but not operationally open. Mythos Preview access is limited to approximately 50 invited organizational partners including Cloudflare, Microsoft, and Oracle. There is no open application pathway. TAC and Glasswing are independently maintained verification programs that are not cross-recognized by the competing organizations."
    section_anchor: anthropics-project-glasswing-and-the-cyber-verification-program
  - question: "A security team with TAC verification deploys Codex with full shell access and no output logging for vulnerability triage. Which governance layers have they addressed and which are uncontrolled?"
    options:
      - "Model access eligibility is addressed via TAC; endpoint governance and agent/tool controls are both uncontrolled"
      - "Endpoint governance is addressed via the TAC API tier; model access eligibility and tool-scope controls are the remaining gaps"
      - "Agent/tool controls are addressed by Codex's built-in sandboxing; model access eligibility is the unresolved gap"
      - "All three governance layers are addressed — TAC verification covers the full governance stack including endpoint and tool controls"
    correct_idx: 0
    explanation: "TAC addresses model access eligibility — the team qualifies for the relaxed output policy on eligible workflow categories. Full shell access with no output logging is an agent/tool control failure: the tool scope is unconstrained and there is no audit trail. Endpoint governance (OpenAI-direct vs. Enterprise vs. Bedrock) is a separate question the example doesn't address. These three layers are independent; TAC eligibility does not imply endpoint controls or tool-scope constraints."
    section_anchor: gpt-55-cyber-in-codex-what-expands-and-what-stays-restricted
tags:
