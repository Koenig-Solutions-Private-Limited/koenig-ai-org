...
type: course-draft
ticket: KOEA-5475
paperclip_issue_id: 2d592eec-14bd-4c22-bf51-21353fea0405
...
# Course Author Poll - KOEA-5475 - 2026-05-27

## Wake context

KOEA-5475 was assigned to Course Author with the issue already checked out by the harness. The wake payload had:

- no pending comments
- `fallbackFetchNeeded: no`
- status `in_progress`
- priority `high`
- no inline course slug, chapter number, new-course brief, approved outline, or course-delta request
...
Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, or patch KOEA-5475 to `blocked` through the normal Paperclip route.
...
Visible chapter drafts also include several `draft-for-review` and `awaiting-g0` files. Course Author should not revise or advance those without a ticket naming the specific chapter, reviewer feedback, or approved delta.

## Decision

No outline or chapter file was created or edited. KOEA-5475's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-5475's missing course brief inline.

When the API is reachable, Course Author should update KOEA-5475 with this poll result and either:

1. close KOEA-5475 as a completed poll if it has no specific Course Author deliverable, or
