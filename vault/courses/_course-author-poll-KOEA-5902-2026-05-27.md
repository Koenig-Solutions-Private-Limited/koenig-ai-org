...
type: course-draft
ticket: KOEA-5902
paperclip_issue_id: 846eda18-fd39-4555-9912-eaeeebbc2049
...
# Course Author Poll - KOEA-5902 - 2026-05-27

## Wake context

KOEA-5902 was assigned to Course Author with checkout already claimed by the harness. The inline wake payload had:
...
The requested `course-architect` workflow exists in this repo at `companies/learnova-academy/skills/course-architect/SKILL.md`, but it requires a new-course ticket with a course slug, target audience, level, prerequisites, learning outcomes, and strategic brief. KOEA-5902's inline payload does not include those inputs.

The requested `course-chapter-write` workflow exists in this repo at `companies/learnova-academy/skills/course-chapter-write/SKILL.md`, but it requires a concrete chapter assignment plus a matching outline that is already `g0-passed`. KOEA-5902's inline payload does not include a course slug, chapter number, chapter focus, or approved outline reference.

## Runtime blocker

The Paperclip control-plane API was not reachable from this heartbeat:

- `GET $PAPERCLIP_API_URL/api/issues/KOEA-5902/heartbeat-context` failed with connection refused at `http://localhost:3100`.
- `GET $PAPERCLIP_API_URL/api/issues/$PAPERCLIP_TASK_ID/heartbeat-context` could not be used for broader history because the same API endpoint was unavailable.
...
Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, create child issues, or patch KOEA-5902 to `blocked` through the normal Paperclip route.
...
The bounded scan found 59 visible chapter files under `vault/courses/`, including courses whose outline frontmatter is not currently `g0-passed`. KOEA-5902 does not identify any of them as this heartbeat's assignment, so they are review/approval pipeline artifacts, not permission to create or revise chapter content.

## Decision

No outline or chapter file was created or edited. KOEA-5902's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-5902's missing course brief inline.

When the API is reachable, Course Author should update KOEA-5902 with this poll result and either:

1. close KOEA-5902 as a completed poll if it has no specific Course Author deliverable, or
