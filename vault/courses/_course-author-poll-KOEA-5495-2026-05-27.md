...
type: course-draft
ticket: KOEA-5495
paperclip_issue_id: 32c8903f-eff2-400a-a720-a91100f6f934
...
# Course Author Poll - KOEA-5495 - 2026-05-27

## Wake context

KOEA-5495 was assigned to Course Author with checkout already claimed by the harness. The inline wake payload had:
...
The requested `course-architect` workflow is available locally at `companies/learnova-academy/skills/course-architect/SKILL.md`, but it requires a new-course ticket with a course slug, target audience, level, prerequisites, learning outcomes, and strategic brief. KOEA-5495's inline payload does not include those inputs.

The requested `course-chapter-write` workflow is available locally at `companies/learnova-academy/skills/course-chapter-write/SKILL.md`, but it requires a concrete chapter assignment plus a matching outline that is already `g0-passed`. KOEA-5495's inline payload does not include a course slug, chapter number, chapter focus, or approved outline reference.

## Runtime blocker

The Paperclip control-plane API was not reachable from this heartbeat:

- `GET $PAPERCLIP_API_URL/api/issues/KOEA-5495/heartbeat-context` failed with connection refused at `http://localhost:3100`.
- `GET http://localhost:3100/api/health` failed with connection refused.
- `GET http://localhost:3101/api/health` failed with connection refused.
- `GET http://localhost:3102/api/health` failed with connection refused.
- `GET http://localhost:3000/api/health` failed with connection refused.

Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, create child issues, or patch KOEA-5495 to `blocked` through the normal Paperclip route.
...
Visible chapter drafts and course artifacts may exist below these folders, but KOEA-5495 does not identify any specific chapter requiring Course Author action.

## Decision

No outline or chapter file was created or edited. KOEA-5495's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-5495's missing course brief inline.

When the API is reachable, Course Author should update KOEA-5495 with this poll result and either:

1. close KOEA-5495 as a completed poll if it has no specific Course Author deliverable, or
