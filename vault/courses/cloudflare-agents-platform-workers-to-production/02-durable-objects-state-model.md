...
tags:
  ...
  - 2026
quiz:
  - question: "What distinguishes a Durable Object from a standard Cloudflare Worker?"
    options:
      - "It runs faster by skipping network latency on all outbound binding calls"
      - "It has a globally unique identity routing all requests to one specific instance"
      - "It runs on dedicated servers rather than shared Cloudflare edge infrastructure"
      - "It persists JavaScript instance variables automatically across all requests"
    correct_idx: 1
    explanation: "A DO's defining property is a globally unique identity that guarantees all requests for a given name route to the same V8 isolate. Workers are stateless; DO state is in SQLite storage (not JS instance variables); DOs run on the same shared Cloudflare edge as Workers."
    section_anchor: the-mental-model-what-a-durable-object-actually-is
  - question: "What happens to a Durable Object's JavaScript instance variables when the instance hibernates?"
    options:
      - "They are serialized to SQLite storage automatically and restored on the next wake"
      - "They are written to Workers KV as a fallback persistence layer during eviction"
      - "They are lost; only data written to ctx.storage survives hibernation cycles"
      - "They remain in memory indefinitely since Cloudflare never evicts active isolates"
    correct_idx: 2
    explanation: "V8 isolate teardown during hibernation destroys all JavaScript instance-level state. Only data written to ctx.storage (SQLite or transactional storage) persists across hibernation. This is the most common source of first-time DO bugs."
    section_anchor: the-lifecycle-in-detail-creation-active-hibernating-and-destroyed
  - question: "A Cloudflare agent needs daily memory consolidation without a live user request. What is the correct mechanism?"
    options:
      - "Register a Cloudflare Cron Trigger to invoke the DO via a Worker on schedule"
      - "Use setInterval inside the DO constructor to fire consolidation every 24 hours"
      - "Call ctx.storage.setAlarm with a future timestamp and handle work in alarm()"
      - "Schedule the consolidation from an external cron job dispatched via Queue messages"
    correct_idx: 2
    explanation: "ctx.storage.setAlarm() schedules a future alarm that wakes the DO even from hibernation. setInterval is destroyed when the isolate hibernates. External cron triggers add unnecessary infrastructure; the DO's built-in alarm API is the native, zero-overhead pattern."
    section_anchor: alarms-your-agents-built-in-scheduler
  - question: "In the Facets pattern, how does a Worker identify which Durable Object instance handles a user's request?"
    options:
      - "The Worker queries a session-to-instance mapping table in D1 for each request"
      - "The Worker calls a load balancer that selects the least-loaded DO instance"
      - "The Worker derives the DO ID from the session identifier using idFromName()"
      - "The Worker creates a new DO instance per request and re-injects prior state"
    correct_idx: 2
    explanation: "idFromName(sessionId) produces a deterministic, stable DO ID for any session identifier. No mapping table, no load balancer, no secondary lookup. The same name always routes to the same instance — the Facets pattern collapses identity resolution into a single call."
    section_anchor: the-facets-pattern-per-user-isolation-at-scale
---
