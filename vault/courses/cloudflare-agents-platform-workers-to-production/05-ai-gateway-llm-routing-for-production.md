...
tags:
  ...
  - 2026
quiz:
  - question: "What does adding AI Gateway to a Workers agent change at the code level?"
    options:
      - "The agent class must extend AiGatewayAgent instead of the standard Agent base class"
      - "All agent tool methods need a gateway wrapper to log their execution time"
      - "Only the model call URL or gateway option changes; agent logic stays the same"
      - "A new wrangler.toml binding for the gateway replaces the existing AI model bindings"
    correct_idx: 2
    explanation: "For Workers AI: add a gateway option to env.AI.run(). For OpenAI SDK: change baseURL. All existing streaming, function calling, and response parsing logic is unchanged. AI Gateway is a routing layer, not a code framework — the minimal code change is intentional."
    section_anchor: architecture-how-ai-gateway-fits-into-a-workers-agent
  - question: "How does AI Gateway semantic caching differ from standard HTTP exact-match caching?"
    options:
      - "Semantic caching stores responses in Workers KV while HTTP caching uses Cloudflare's CDN"
      - "Semantic caching matches queries by vector similarity, not just identical text strings"
      - "Semantic caching only applies to Workers AI models, not external OpenAI endpoints"
      - "Semantic caching invalidates immediately after each response while HTTP caching respects TTLs"
    correct_idx: 1
    explanation: "AI Gateway embeds incoming prompts and compares them against cached entries by vector similarity. 'What is your refund policy?' and 'How do I get my money back?' can hit the same cache entry. Exact-match HTTP caching would miss the second query. Cache hit rates of 20–40% are typical for support agents."
    section_anchor: step-4-configure-semantic-caching
  - question: "What event triggers fallback routing to a secondary model in AI Gateway?"
    options:
      - "The primary provider's response time exceeds the gateway's configured latency threshold"
      - "The semantic cache reports a miss and the primary model's request queue overflows"
      - "The primary model provider returns a 5xx server error to the gateway"
      - "The current session's rate limit is exceeded on the primary model endpoint"
    correct_idx: 2
    explanation: "Fallback routing activates automatically when the primary provider returns a 5xx error. The gateway retries the same request against the fallback provider and returns the result transparently to your Worker. Latency thresholds, cache misses, and rate limits trigger different behaviors, not fallback routing."
    section_anchor: step-6-configure-fallback-routing
  - question: "A high P99 latency with a low P50 in AI Gateway analytics typically indicates what?"
    options:
      - "Low cache hit rate forces most requests to reach the model provider directly"
      - "The gateway itself adds overhead that grows proportionally with request volume"
      - "Most requests complete quickly but occasional ones hit LLM provider-side slowness"
      - "Fallback routing is activating frequently, doubling the latency of affected requests"
    correct_idx: 2
    explanation: "Low P50 with high P99 is classic tail latency — most requests complete quickly but a small percentage hit slowness at the provider level. AI Gateway adds <1ms overhead, so gateway latency is not the cause. Cache issues raise P50 as well. Fallback activation appears as a distinct latency cluster."
    section_anchor: step-7-reading-the-analytics-dashboard
---
