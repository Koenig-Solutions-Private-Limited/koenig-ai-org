...
tags:
  ...
  - 2026
quiz:
  - question: "What is the key latency advantage of Workers bindings over HTTP-endpoint tools?"
    options:
      - "Bindings automatically cache responses to reduce latency on repeated tool calls"
      - "Binding calls have zero additional network latency since they execute inside the Worker"
      - "Bindings allow calling external services while HTTP tools only reach internal APIs"
      - "Bindings auto-retry on failure while HTTP endpoint tools need manual retry logic"
    correct_idx: 1
    explanation: "Workers binding calls (D1, R2, KV, Queue) execute within the Cloudflare network co-located with your Worker — no public internet round-trip, no auth token overhead. For a 5-tool agent chain, cutting per-tool latency from 50–100ms (HTTP) to 5–10ms (binding) saves 250–500ms per chain."
    section_anchor: the-workers-tool-model-vs-http-tools
  - question: "Why should @tool-decorated method return values be plain strings rather than JSON objects?"
    options:
      - "The Agents SDK cannot serialize JSON objects returned from decorated tool methods"
      - "JSON objects cause deserialization errors inside the LLM's active context window"
      - "Strings let the LLM reason directly about results and recover from tool failures"
      - "The @tool decorator strips all JSON keys and only serializes primitive values"
    correct_idx: 2
    explanation: "When a tool returns a descriptive string, the LLM can act on the result or attempt an alternative. When a tool throws or returns raw JSON, the LLM sees a generic error or raw data and cannot reason about recovery. Descriptive strings are the correct interface for LLM-readable tool output."
    section_anchor: defining-tools-with-tool-and-zod
  - question: "How does Cloudflare Workers enforce tool sandboxing for agent bindings by construction?"
    options:
      - "Each tool method checks a permissions list before accessing any binding at runtime"
      - "Workers isolates each tool call in a separate V8 isolate for security enforcement"
      - "A Worker can only access bindings declared in its own wrangler.toml configuration"
      - "The Agents SDK validates each tool call against an internal allowed-tools allowlist"
    correct_idx: 2
    explanation: "Binding scoping is enforced by the runtime: a Worker can only access bindings declared in its own wrangler.toml. No cross-Worker ambient access, no runtime permissions list needed. Sandboxing is a property of the deployment configuration, not of application code."
    section_anchor: tool-sandboxing-what-workers-gives-you-by-default
  - question: "A Cloudflare agent tool encounters a D1 database error mid-execution. What is the correct handling pattern?"
    options:
      - "Throw an exception so the Agents SDK logs and escalates the error automatically"
      - "Return null so the Agents SDK falls back to a configured default tool response"
      - "Return a descriptive error string so the LLM can reason about the failure"
      - "Call connection.send directly with the error message before returning from the method"
    correct_idx: 2
    explanation: "When a tool returns a descriptive error string, the LLM can recover — try an alternative approach, ask for clarification, or escalate. A thrown exception surfaces as a generic 'Tool execution failed' message the LLM cannot reason about. Error strings are the correct interface for graceful agent recovery."
