...
tags:
  ...
  - 2026
quiz:
  - question: "What is the correct way to propagate a trace ID into a Cloudflare Workflow for end-to-end tracing?"
    options:
      - "Store the traceId in Workers KV and retrieve it inside each Workflow step by key"
      - "Include the traceId as a named field in the Workflow params object at WORKFLOW.create()"
      - "Log the traceId at Worker entry and rely on Cloudflare's built-in log correlation"
      - "Attach the traceId as a custom HTTP header on every Workflow internal step call"
    correct_idx: 1
    explanation: "Include the traceId in the params object when calling env.WORKFLOW.create(). The Workflow receives it as event.payload.traceId and can log it at every step. KV lookup adds latency and complexity; Cloudflare does not auto-correlate trace IDs across contexts; Workflow steps have no HTTP headers to attach."
    section_anchor: distributed-trace-propagation-across-worker-workflow-do
  - question: "How does Workers Analytics Engine differ from console.log for production agent observability?"
    options:
      - "console.log captures stack traces while Analytics Engine stores only top-level event names"
      - "Analytics Engine stores structured numeric metrics queryable via GraphQL; console.log produces text logs only"
      - "console.log is free while Analytics Engine counts against the Workers billable request quota"
      - "Analytics Engine writes synchronously and blocks the Worker; console.log is fully asynchronous"
    correct_idx: 1
    explanation: "Workers Analytics Engine stores structured data points (blobs + doubles + indexes) queryable via GraphQL for aggregations like token spend per model per day. console.log produces unstructured text searchable via Logpush but not aggregatable. Analytics Engine is the correct tool for time-series cost and latency dashboards."
    section_anchor: workers-analytics-engine-for-custom-metrics
  - question: "Which prompt injection defense is the most structurally important for a Cloudflare agent?"
    options:
      - "Add a regex filter that strips all punctuation from user input before LLM calls"
      - "Validate tool call arguments with a Zod schema before each tool method executes"
      - "Keep user input in the user role message, never injected into the system prompt"
      - "Run every LLM call inside a Cloudflare Workflow step to isolate its execution"
    correct_idx: 2
    explanation: "Role isolation — user input only in the user role, never in the system prompt — is the primary structural defense. Injecting user text into the system prompt lets it override agent instructions. Regex filters miss novel patterns. Zod schemas protect tool arguments but not the system prompt. Workflow isolation is for durability, not security."
    section_anchor: prompt-injection-defense
  - question: "What is the Durable Object SQLite storage limit per instance, and how are application-level memory budgets enforced?"
    options:
      - "1 GB; the platform automatically evicts oldest rows when the threshold is reached"
      - "10 GB; enforced by a scheduled DO alarm that checks usage and evicts old history"
      - "100 MB; set with PRAGMA max_page_count on the SQLite instance in the constructor"
      - "Unlimited; Workers account billing caps enforce the only real storage constraint"
    correct_idx: 1
    explanation: "The platform limit is 10 GB per DO SQLite instance with no automatic eviction — you must implement your own budget. Schedule a DO alarm that checks history length on a cadence and evicts the oldest rows when the threshold is exceeded. This is the only mechanism that runs during hibernation with no live user connection."
    section_anchor: do-memory-budgets-with-alarms
---
