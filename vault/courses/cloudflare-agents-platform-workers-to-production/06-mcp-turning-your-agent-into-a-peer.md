...
tags:
  ...
  - 2026
quiz:
  - question: "What problem does the Model Context Protocol (MCP) solve for Cloudflare Workers agents?"
    options:
      - "It compresses LLM context windows to fit more tool schemas into one request"
      - "It allows external LLM clients to call your agent's tools using a standard protocol"
      - "It encrypts tool call arguments end-to-end between the LLM and agent bindings"
      - "It synchronizes state between multiple Durable Object instances serving one session"
    correct_idx: 1
    explanation: "MCP provides a standard JSON-RPC protocol for tool discovery and invocation. External clients (Claude Desktop, Cursor, other agents) can call your Workers agent's tools without you building a custom API. The agent becomes a capability provider in a multi-agent network, accessible from any MCP-compatible client."
    section_anchor: what-mcp-is-and-why-it-changes-the-agent-architecture
  - question: "When you change a Cloudflare agent from Agent to McpAgent, which methods are exposed via MCP?"
    options:
      - "All public methods on the agent class regardless of how they are defined"
      - "All methods decorated with @tool are listed and callable via the MCP protocol"
      - "Only methods explicitly registered in a separate MCP server configuration object"
      - "Methods named tools/list and tools/call if they exist as defined class methods"
    correct_idx: 1
    explanation: "McpAgent extends Agent and wires the MCP protocol to @tool-decorated methods automatically. No separate MCP config is required. tools/list returns all @tool methods with their descriptions and schemas; tools/call invokes the chosen method. Public methods without @tool are not exposed."
    section_anchor: mcpagent-the-agents-sdks-mcp-server-primitive
  - question: "Why is a Cloudflare Access service token preferable to Worker-code API key validation for an MCP endpoint?"
    options:
      - "Service tokens rotate automatically while Worker-code key validation requires manual rotation"
      - "Cloudflare Access blocks unauthenticated requests at the edge before the Worker executes"
      - "Service tokens work with Claude Desktop while Worker-level API keys do not integrate"
      - "Access policies can be updated without redeploying the Worker, unlike inline key checks"
    correct_idx: 1
    explanation: "Cloudflare Access enforces auth at the edge — unauthenticated requests never reach your Worker code, never consume CPU, and never touch your bindings. Worker-code validation still accepts the request and runs the Worker. Edge enforcement is both cheaper and more secure than application-layer key checking."
    section_anchor: step-5-secure-with-cloudflare-access-service-tokens
  - question: "In a combined MCP + A2A architecture, which protocol should handle full end-to-end case delegation?"
    options:
      - "MCP tools/call to invoke a single tool that chains all downstream steps internally"
      - "A2A task delegation to assign the complete task and receive structured output on completion"
      - "WebSocket chat message interpreted by the agent as an instruction to start the workflow"
      - "HTTP POST directly to the Workflow binding to bypass the agent and start execution"
    correct_idx: 1
    explanation: "A2A (Agent-to-Agent) is designed for complete task delegation — the orchestrator assigns a structured task and receives a structured result when done. MCP is for direct tool invocation (individual tool calls, not end-to-end tasks). The Workflow runs under the hood; the A2A protocol surfaces its result to the caller."
    section_anchor: combining-mcp-with-a2a-for-multi-agent-workflows
---
