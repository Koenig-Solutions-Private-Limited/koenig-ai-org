...
tags:
  ...
  - 2026
quiz:
  - question: "Which of the four Cloudflare Agents Platform primitives provides per-agent persistent state?"
    options:
      - "Workers — the global stateless compute layer that routes incoming requests"
      - "AI Gateway — the LLM routing and semantic caching proxy for model calls"
      - "Durable Objects — the per-agent persistent SQLite state layer"
      - "Workflows v2 — the durable multi-step execution engine for long tasks"
    correct_idx: 2
    explanation: "Durable Objects are the state layer. Each DO instance has its own SQLite database co-located with its compute. Workers are stateless by design; AI Gateway handles LLM routing; Workflows handle durable multi-step execution."
    section_anchor: the-four-primitives
  - question: "In a Cloudflare Workers agent architecture, what is the stateless Worker's primary role?"
    options:
      - "Stores conversation history directly in module-level variables between requests"
      - "Routes requests to the correct Durable Object instance by user session ID"
      - "Executes all LLM calls and persists the results to Workers KV storage"
      - "Manages the Durable Object lifecycle including creation and memory eviction"
    correct_idx: 1
    explanation: "The Worker is stateless — it routes each request to the correct Durable Object by session ID. All state lives in the DO. Module-level variables are lost between invocations; the Worker does not manage the DO lifecycle."
    section_anchor: stateless-workers-vs-stateful-agents-the-mental-model-shift
  - question: "Which scenario is a case where Cloudflare Agents is NOT the right architecture choice?"
    options:
      - "Your agent needs sub-100ms wake time and per-session isolated SQLite state"
      - "You want a single deployment serving all 330+ global Cloudflare edge locations"
      - "Your agent needs SQL joins across all user sessions in a single query"
      - "You need durable multi-step execution that survives transient API failures"
    correct_idx: 2
    explanation: "DO SQLite is per-instance — there is no API to query across all user sessions simultaneously. For cross-session relational queries, use D1 or an external database. The other three scenarios are exactly what Cloudflare Agents excels at."
    section_anchor: when-to-choose-cloudflare-agents-over-alternatives
  - question: "What does `routeAgentRequest()` from the Cloudflare Agents SDK do?"
    options:
      - "Validates all incoming tool arguments against their registered Zod schemas at runtime"
      - "Routes WebSocket and HTTP requests to the correct Durable Object agent instance"
      - "Initializes the SQLite schema in a newly created Durable Object on first request"
      - "Proxies all outgoing LLM calls through AI Gateway for unified cost tracking"
    correct_idx: 1
    explanation: "routeAgentRequest() routes WebSocket upgrades and HTTP requests to the correct Agent Durable Object instance based on the session ID in the URL path. Zod validation, SQLite initialization, and AI Gateway proxying are separate SDK concerns."
    section_anchor: the-agents-sdk-what-it-gives-you
---
