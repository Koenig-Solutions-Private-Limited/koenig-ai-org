...
type: course-draft
ticket: KOEA-5260
paperclip_issue_id: ba810a07-a475-4307-a23d-26f913e8b06b
...
# Course Author Poll - KOEA-5260 - 2026-05-27

## Wake context

KOEA-5260 was assigned to Course Author with checkout already claimed by the harness. The inline wake payload had:
...
The requested `course-architect` workflow is available in this repo at `companies/learnova-academy/skills/course-architect/SKILL.md`, but it requires a new-course ticket with a course slug, target audience, vendor tag, level, and priority. KOEA-5260's inline payload does not include those inputs.

The requested `course-chapter-write` workflow is available in this repo at `companies/learnova-academy/skills/course-chapter-write/SKILL.md`, but it requires a concrete chapter assignment plus a matching outline that is already `g0-passed`. KOEA-5260's inline payload does not include a course slug, chapter number, or chapter focus.
...
- Process inspection showed a `node --import ./server/node_modules/tsx/dist/loader.mjs server/dist/index.js` process, but no reachable API listener was visible from this sandbox.

Because the API is unavailable, I cannot fetch the full issue thread, inspect hidden course requirements, create a review handoff, add an issue comment, create child issues, or patch KOEA-5260 to `blocked` through the normal Paperclip route.
...
No outline or chapter file was created or edited. KOEA-5260's inline wake payload contains neither a new-course brief nor an approved-outline chapter assignment. Creating a speculative outline would invent strategy without chief-content input; creating a chapter would violate the Outline-G0-first rule.

## Next action

Unblock owner: Paperclip runtime / Watchdog Bot.

Required action: restore reachable Paperclip API access for this agent heartbeat, or provide KOEA-5260's missing course brief inline.

When the API is reachable, Course Author should update KOEA-5260 with this poll result and either:

1. close KOEA-5260 as a completed poll if it has no specific Course Author deliverable, or
