---
title: "AI Agent Observability with Langfuse in 2026 — Production Setup"
...
positions:
  - stance: cost-fix-the-leak
    angle: "Langfuse's ingested cost model exposes exactly where agent spending leaks — ingested usage data wins over inferred pricing, so cost anomalies surface before they compound into billing surprises"
  - stance: open-closed-prefer-oss
    angle: "Langfuse is MIT-licensed and fully self-hostable; for teams that cannot send prompt data to a SaaS vendor, it is the only production-grade LLM observability stack that doesn't require a vendor relationship"
whats_new:
...
faq:
  - question: "What is Langfuse?"
    answer: "Langfuse is an open-source LLM observability platform (MIT-licensed) that provides self-hosted tracing, cost tracking, prompt management, and OTLP-native ingestion for AI agent systems. It runs as a six-container stack — web, worker, Postgres, ClickHouse, Redis, and MinIO — and accepts traces from any OTLP-compatible agent framework."
  - question: "How do you instrument AI agents with Langfuse?"
    answer: "Point your agent's OTLP exporter at Langfuse's `/api/public/otel` endpoint, authenticate with Basic Auth using your `public_key:secret_key`, and propagate `user.id`, `session.id`, and tags on every span. For Claude Code, set `CLAUDE_CODE_ENABLE_TELEMETRY=1` and configure `OTEL_EXPORTER_OTLP_ENDPOINT`. For Codex, add an `[otel]` block to `~/.codex/config.toml`. For custom agents, use the OpenTelemetry SDK directly."
  - question: "Is Langfuse free for self-hosting?"
    answer: "Yes. Langfuse's self-hosted deployment is free under the MIT license with no seat or usage fees. You pay only for the infrastructure you run (Postgres, ClickHouse, Redis, MinIO, and two application containers). Langfuse Cloud has a free tier and paid plans; self-hosting removes the SaaS cost entirely and is the right path when data residency or prompt privacy is a requirement."
---

# Run Langfuse as the observability backend for mixed agent stacks in 2026

To set up Langfuse for production AI agent observability in 2026, run the six-container self-hosted stack (web, worker, Postgres, ClickHouse, Redis, MinIO), export OTLP traces from your coding agents, and propagate user, session, and cost metadata on every span. Langfuse is open-source under MIT, fully self-hostable, and OTEL-native — the strongest pick when you run mixed agent stacks across Claude Code, Codex, and similar tools.[1][6][7]
...
<RunPromptCell
  prompt="curl -sf http://localhost:3200/api/public/health"
...
<RunPromptCell
  prompt={`export CLAUDE_CODE_ENABLE_TELEMETRY=1
...
claude`}
  expectedOutput="Claude starts and OTEL spans appear in Langfuse under the claude-code service name"
/>
<!-- TODO: verify expected output with QA -->
...
<Callout type="warn">
Cost dashboards built on inferred token counts rather than provider-reported `usage_details` will undercount or miscount for any model where token inference diverges from actual billing. Always ingest `cost_details` from your provider responses, not just token counts.[3]
</Callout>
...
|---|---|---|---|---|
| Open source posture | Yes (MIT) | No | Partial | Phoenix OSS alongside enterprise platform |
...
<KnowledgeCheck
  question="Why does Langfuse recommend ingesting provider-reported cost_details rather than relying on token inference?"
  options={[
    "Because inferred pricing is always zero",
    "Because Langfuse cannot read token counts from spans",
    "Because ingested cost data wins over inferred values, making it the only reliable source for billing and alert thresholds",
    "Because cost_details is required by the OTLP specification"
  ]}
  correctIdx={2}
  explanation="Langfuse's docs are explicit: ingested usage and cost data take priority over inferred pricing. Token inference is a fallback, not a billing truth source.[3]"
/>
