...
This path fits internal demos, localized agent voices, game prototypes, and low-risk support personas. It is weaker for executive replicas, premium narration, digital twins, or regulated customer flows where the listener will notice room tone, pacing artifacts, or style drift.
...
If cloning is only the first step, the harder system is the realtime agent around it: streaming transport, interruption handling, tool calls, and evaluation. Build that layer in [[course/building-realtime-voice-agents]] after you decide whether Sonic 3 instant cloning or PVC is the right voice asset.
