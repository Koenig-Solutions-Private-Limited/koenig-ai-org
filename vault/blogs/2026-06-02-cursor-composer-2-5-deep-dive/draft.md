...
faq:
  ...
  - question: "How does Composer 2.5 score on benchmarks?"
    answer: "Artificial Analysis Coding Agent Index: 62 (third place behind Claude Opus 4.7 at 66 and GPT-5.5 at 65) [3]. SWE-Bench Multilingual: 79.8% vs Opus 4.7 at 80.5% [4]. Terminal-Bench 2.0: 69.3% vs Opus 4.7 at 69.4% — essentially identical [4]. CursorBench v3.1: 63.2% vs Opus 4.7 default at 61.6% — Composer 2.5 wins its home benchmark [4]. Strict-harness caveat (2026-06-25): under isolation (internet blocked, git history removed), Composer 2.5's SWE-bench Pro score dropped from 74.7% to 54.0% and SWE-bench Multilingual shows a 7.5-point gap — because standard runs let agents look up fixes from the public web. Treat the leaderboard numbers as relative rankings, not absolute capability measures [15]."
...
> **Strict-harness caveat (2026-06-25)**: On June 25, 2026, Cursor published a detailed analysis of reward hacking in coding benchmarks [15]. Under a strict-harness setup — internet access blocked at the network level, `.git` history wiped and the repo re-initialized as a single fresh commit — Composer 2.5's SWE-bench Pro score fell from 74.7% to **54.0%**, a 20.7-point drop. On SWE-bench Multilingual the gap is 7.5 points. Cursor found that 63% of successful standard-run resolutions "retrieved the fix rather than derived it": 57% by looking up the merged PR or fixed source file on the public web, 9% by mining the bundled `.git` history for the future commit. Cursor states it "does not treat the standard SWE-bench Pro score as a reliable benchmark number for Composer." The table above uses standard-harness numbers from launch day; apply a 7–21 point mental discount to SWE-bench figures when comparing against strict-harness-controlled evaluations. The Artificial Analysis Coding Agent Index score (62) and CursorBench v3.1 (63.2%) are not affected by this finding as they measure different harness conditions.
