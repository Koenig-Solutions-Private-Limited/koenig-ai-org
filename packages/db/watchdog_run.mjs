...
const apiPatch=async (p,b)=>{const r=await fetch(`${API}${p}`,{method:'PATCH',headers:{Authorization:`Bearer ${KEY}`,'Content-Type':'application/json'},body:JSON.stringify(b)});if(!r.ok)throw new Error(`PATCH ${p} ${r.status} ${await r.text()}`);return r.json();};
...
if(fs.existsSync(log)){
  ...
  if(m){const d=new Date(m[1].replace(' UTC','Z').replace(' ','T'));if(!Number.isNaN(+d)){lastTick=d.toISOString();stale=(NOW-d)>10*60*1000;}}
}
if(stale){
  ...
  if(!dup){
    ...
    await apiPost(`/api/companies/${CID}/issues`,{title,description:'Watchdog detected no publish-action success tick in last 10 minutes.',priority:'critical',status:'blocked',assigneeAgentId:chief?.id});
  }
...
const staleRows=await listStaleBlockedNudgeCandidates(sql,{companyId:CID,now:NOW,staleDays:7,limit:20});
for(const s of staleRows){
  // Revalidate each candidate immediately before commenting to avoid stale snapshots.
  const freshCandidate=await listStaleBlockedNudgeCandidates(sql,{companyId:CID,now:NOW,staleDays:7,limit:1,issueId:s.id});
  if(!freshCandidate.length) continue;
  const assignee=agents.find(x=>x.id===s.assignee_agent_id);
  const mention='@'+((assignee?.name||'chief-engineering').toLowerCase().replace(/\s+/g,'-'));
  if(await recentSelf(s.id,'this has been blocked >7 days')) continue;
  await apiPost(`/api/issues/${s.id}/comments`,{body:`${mention} this has been blocked >7 days with no movement. Cancel if obsolete, or escalate via daily-engineering-triage / Chief Content / etc.`});
}
if(!(await recentSelf(meta.id,'Stale-blocked tickets (>7d):'))){
  await apiPost(`/api/issues/${meta.id}/comments`,{body:`Stale-blocked tickets (>7d): ${staleRows.length}.\n${formatStaleBlockedDigest(staleRows)}`});
}

const cleanupCandidates=await listSupersededHourlyDispatchCleanupCandidates(sql,{companyId:CID,now:NOW,staleDays:7,limit:50});
...
if(cleanupApply){
  for(const c of cleanupCandidates){
    const freshCleanup=await listSupersededHourlyDispatchCleanupCandidates(sql,{companyId:CID,now:NOW,staleDays:7,limit:1,issueId:c.id});
    if(!freshCleanup.length) continue;
    await apiPost(`/api/issues/${c.id}/comments`,{body:`Watchdog cleanup apply: cancelling superseded stale hourly dispatch issue. Superseded by ${c.superseding_identifier} (${c.superseding_status}).`});
    await apiPatch(`/api/issues/${c.id}`,{status:'cancelled'});
    cleanupApplied++;
  }
} else if(!(await recentSelf(meta.id,'stale-blocked cleanup dry-run='))){
  await apiPost(`/api/issues/${meta.id}/comments`,{body:`stale-blocked cleanup dry-run=${cleanupCandidates.length}\n${cleanupCandidates.map(c=>`- ${c.identifier} -> ${c.superseding_identifier} (${c.superseding_status})`).join('\n')||'none'}`});
}
