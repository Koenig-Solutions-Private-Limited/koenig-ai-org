...
({publishStale:stale,lastSuccessAt,evidenceLine}=getPublishHeartbeatEvidence(log));
if(stale){
  publish='FAILING (alerting)';
  const title=`${PUBLISH_ALERT_TITLE_PREFIX} — last_success_at: ${lastSuccessAt}`;
  const dup=issues.find(i=>
    ACTIVE_DEDUPE_STATUSES.has(i.status) &&
    typeof i.title==='string' &&
    i.title.startsWith(PUBLISH_ALERT_TITLE_PREFIX)
  );
  if(!dup){
    ...
    await apiPost(`/api/companies/${CID}/issues`,{
      title,
      description:`Watchdog detected no publish-action success completion in last 10 minutes.\n\nlast_success_at: ${lastSuccessAt}\nevidence_line: ${evidenceLine}`,
      priority:'critical',
      status:'blocked',
      assigneeAgentId:chief?.id,
    });
  }
}else{publish=`OK (last success: ${Math.floor((NOW-new Date(lastSuccessAt))/60000)}min ago)`;}
