...
const API=process.env.PAPERCLIP_API_URL, KEY=process.env.PAPERCLIP_API_KEY, CID=process.env.PAPERCLIP_COMPANY_ID, SELF=process.env.PAPERCLIP_AGENT_ID, TASK=process.env.PAPERCLIP_TASK_ID;
const NOW=new Date(), CD=4*60*60*1000;
const sql=postgres(process.env.DATABASE_URL);
const ACTIVE_DEDUPE_STATUSES=new Set(['backlog','todo','in_progress','in_review','blocked']);
const PUBLISH_ALERT_TITLE_PREFIX='[WATCHDOG] publish-action.sh silent >10min';

const issues=await (await fetch(`${API}/api/companies/${CID}/issues?limit=800`,{headers:{Authorization:`Bearer ${KEY}`}})).json();
const fresh=t=>NOW-new Date(t)<=CD;
const post=async (p,b)=>{const r=await fetch(`${API}${p}`,{method:'POST',headers:{Authorization:`Bearer ${KEY}`,'Content-Type':'application/json'},body:JSON.stringify(b)});if(!r.ok)throw new Error(`${p} ${r.status} ${await r.text()}`);return r.json();};
const safePost=async (p,b)=>{try{return await post(p,b);}catch{return null;}};
...
const ensureIssue=async(title,description,priority='high')=>{
  ...
  const created=await post(`/api/companies/${CID}/issues`,{title,description,priority,status:'blocked',assigneeAgentId:chief?.id});
  return {created:true,id:created.id};
};
const ensureIssueByPrefix=async(titlePrefix,title,description,priority='high')=>{
  const dup=issues.find(i=>
    ACTIVE_DEDUPE_STATUSES.has(i.status) &&
...
const parsePublishSuccess=(line)=>{
...
const getPublishHeartbeatEvidence=(logPath)=>{
  ...
  return {
    publishStale:(NOW-when)>10*60*1000,
...
if(!meta){meta=await post(`/api/companies/${CID}/issues`,{title:'Watchdog Health',description:'Meta issue for Watchdog heartbeat summaries and fallback alerts.',priority:'medium',assigneeAgentId:SELF});}
...
// 1 publish-action
let lastSuccessAt='none';
let evidenceLine='none';
let publishStale=true;
const log='/paperclip/logs/publish-action.log';
({publishStale,lastSuccessAt,evidenceLine}=getPublishHeartbeatEvidence(log));
if(publishStale){
  publishStatus='FAILING (alerting)';
  if(process.env.TELEGRAM_BOT_TOKEN&&process.env.TELEGRAM_CHAT_ID){
    const txt=encodeURIComponent(`Watchdog alert: publish-action.sh silent >10min. last_success_at: ${lastSuccessAt}. evidence_line: ${evidenceLine}`);
  ...
  await ensureIssueByPrefix(
    PUBLISH_ALERT_TITLE_PREFIX,
    `${PUBLISH_ALERT_TITLE_PREFIX} — last_success_at: ${lastSuccessAt}`,
    `Watchdog detected no publish-action success completion in last 10 minutes.\n\nlast_success_at: ${lastSuccessAt}\nevidence_line: ${evidenceLine}`,
    'critical',
  );
}else publishStatus=`OK (last success: ${Math.floor((NOW-new Date(lastSuccessAt))/60000)}min ago)`;

// 2 approvals
const [{pending_total}]=await sql`select count(*)::int as pending_total from approvals where company_id=${CID} and status='pending'`;
const oldApprovals=await sql`select id,payload->>'issueId' as issue_id from approvals where company_id=${CID} and status='pending' and created_at < now()-interval '48 hours' order by created_at asc`;
for(const a of oldApprovals){
  await ensureIssue(`[WATCHDOG] Approval >48h pending (${a.id})`,`Approval ${a.id} has been pending >48h. Related issueId: ${a.issue_id||'unknown'}. Guidance: cancel if obsolete, else triage in daily-engineering-triage.`,'medium');
}
if(Number(pending_total)>10){
  const r=await ensureIssue('[WATCHDOG] Approval backlog exceeded 10 pending. Operator may want to triage.',`Pending approvals total: ${pending_total}.`,'high');
...
if(missingMarkers.length){
  ...
  await ensureIssue(`[WATCHDOG] Escalation marker compliance gaps — ${missingMarkers.length} comments`,`Marker-missing comments found in blocked issues (last 100 engineering comments):\n${sample}`,'medium');
}

// 4 stale blocked
const staleRows=await sql`select identifier,title,updated_at from issues where company_id=${CID} and status='blocked' and updated_at < now()-interval '7 days' and hidden_at is null order by updated_at asc limit 20`;
if(staleRows.length){
  const sample=staleRows.map(s=>`- ${s.identifier}: ${s.title} (updated ${s.updated_at.toISOString()})`).join('\n');
  await ensureIssue(`[WATCHDOG] Stale blocked tickets >7d (${staleRows.length})`,`These blocked tickets are stale >7 days and need triage:\n${sample}`,'high');
}
...
for(const s of spikes){
  const r=await ensureIssue(`[WATCHDOG] Failure spike: ${s.adapter_type} hitting "${s.signature}" — ${s.fail_count} fails in 1h. Owner: Chief Engineering.`,'Watchdog detected adapter failure spike in heartbeat_runs over the last hour.','high');
...
let scanned=0,found=0,missing=0,recoveryCreated=0,recoveryUpdated=0;
for(const c of candidates){
  scanned++;
  const m=c.title.match(/^\[SLIDES\]\s+([a-z0-9-]+)\s+(ch\d+)/i);
  if(!m) continue;
  const slug=m[1], ch=m[2].toLowerCase();
  const paths=[`/paperclip/tmp/koea1551/koenig-ai-org/vault/courses/${slug}`,...kRoots.map(r=>`${r}/${slug}`),...lRoots.map(r=>`${r}/${slug}`)];
  let fp='';
  for(const p of paths){
    ...
    for(const f of fs.readdirSync(p)){
      if(f.startsWith(`${ch}-slides`)&&f.endsWith('.pptx')){
        ...
        if(fs.statSync(full).size>1000){fp=full;break;}
      }
    }
    if(fp) break;
  }
  if(fp) found++; else missing++;
  const needs=(c.status==='done'&&c.fake_done_audited!=='true')||((c.status==='blocked'||c.status==='reverted')&&c.fake_done_audited==='true'&&!!fp);
  if(!needs) continue;
  ...
  if(!existing){
    await post(`/api/companies/${CID}/issues`,{title,description:body,priority:'high',status:'blocked',assigneeAgentId:(agents.find(a=>a.id===c.assignee_agent_id)||chief)?.id});
    recoveryCreated++;
  } else {
    const updated=await safePost(`/api/issues/${existing.id}/comments`,{body:`Recovery evidence update (${NOW.toISOString()}):\n\n${body}\n\nAuditor recovery — slide file located at ${fp||'none'}. Reverting status back to done.`});
