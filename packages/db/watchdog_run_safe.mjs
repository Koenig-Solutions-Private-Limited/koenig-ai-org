...
const ensureIssue=async(title,description,priority='high',{dedupe='fresh'}={})=>{
  const dup=dedupe==='active'
    ?findActiveIssueByTitle(issues,title)
    :findFreshIssueByTitle(issues,title,NOW,CD);
...
if(fs.existsSync(log)){
  ...
  if(m){const d=new Date(m[1].replace(' UTC','Z').replace(' ','T')); if(!Number.isNaN(+d)){lastTick=d.toISOString(); publishStale=(NOW-d)>10*60*1000;}}
}
if(publishStale){
...
if(Number(pending_total)>10){
  const r=await ensureIssue('[WATCHDOG] Approval backlog exceeded 10 pending. Operator may want to triage.',`Pending approvals total: ${pending_total}.`,'high',{dedupe:'active'});
